...
<head>
    ...
    <title>Progress Tugasan - Studio Manager</title>
...
<body class="bg-slate-50 text-slate-800 text-xs font-sans min-h-screen">
    ...
    <header class="bg-slate-900 text-white sticky top-0 z-30 shadow-md">
        <div class="max-w-7xl mx-auto px-4 py-3.5 flex justify-between items-center">
            <div>
                <h1 class="text-sm font-bold">📝 Checklist Progress Tugasan</h1>
                <p class="text-[11px] text-slate-400"><?= htmlspecialchars($event['event_title'] ?? 'Acara'); ?> • Pelanggan: <?= htmlspecialchars($event['client_name']); ?></p>
            </div>
    ...
    <main class="max-w-4xl mx-auto px-4 py-8 space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-3 h-fit">
                ...
                <form action="api/save_task.php" method="POST" class="space-y-3">
                    ...
                    <div>
                        ...
                        <input type="text" name="task_name" required placeholder="cth: Edit Video Highlight" class="w-full border border-slate-200 rounded-lg p-2 bg-slate-50">
                    </div>

                    <div>
                        <label class="block font-bold mb-1">Tarikh Akhir (Due Date)</label>
                        <input type="date" name="due_date" class="w-full border border-slate-200 rounded-lg p-2 bg-slate-50">
                    </div>
            ...
            <div class="md:col-span-2 bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-3">
                <h3 class="text-sm font-bold text-slate-900 border-b pb-2">📋 Senarai Progress Tugasan</h3>
                ...
                <div class="space-y-2">
                    <?php if ($tasksQuery && $tasksQuery->num_rows > 0): ?>
                            ...
                            <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-200">
                                <div>
                                    ...
                                    <?php if ($task['due_date']): ?>
                                        <p class="text-[10px] text-slate-400">📅 Due: <?= date('d/m/Y', strtotime($task['due_date'])); ?></p>
                                    <?php endif; ?>
                                </div>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <p class="text-slate-400 italic text-center py-4">Belum ada sebarang tugasan dicipta.</p>
                    <?php endif; ?>
                </div>
            </div>
